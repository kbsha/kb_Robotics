...
_SLIDERS = [
    ...
    ("steering_gain",         "steer gain x.01",         50,  150, 0.01),
...
class JetRacerLink(object):
    ...
    # =========================================================
    def update(self, steering_deg, throttle, frame_ok=True, frame_time=None):
        """
        ...
        frame_time             : time the newest video frame arrived (optional,
                                 pass `last_frame_time`). Lets the pulse cycle
    ...
    def _auto(self, now, steering_deg, throttle, frame_ok, frame_time):
        ...
        if not self.pulse_mode:
            return self.map_steering(steering_deg), self.map_throttle(throttle), 0.0
