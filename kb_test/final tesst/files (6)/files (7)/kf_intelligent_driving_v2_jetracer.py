...
JETRACER_IP = "10.0.125.61"     # <-- IP of the JetRacer (run `hostname -I` on it)
...
# --- Black obstacle detector ---
OBSTACLE_HSV_LO   = np.array([0,   0,   0])   # HSV lower bound for "black"
OBSTACLE_HSV_HI   = np.array([180, 255, 50])  # V≤50 catches very dark surfaces
OBSTACLE_ROI_X    = (0.25, 0.75)              # horizontal slice (fraction of width)
OBSTACLE_ROI_Y    = (0.45, 1.00)              # vertical slice  (fraction of height)
OBSTACLE_MIN_AREA = 2500                       # min contour area in pixels (tune for 1280×720)
OBSTACLE_ROI_FRAC = 0.70                       # contour bottom must reach this fraction of ROI
...
obstacle_detected = False   # set by black_obstacle_detected() each frame
...
def black_obstacle_detected(frame):
    """
    Returns True if a large, very dark object is close in front of the car.

    How it works
    ─────────────
    1. ROI   – only looks at the bottom-centre of the frame, ignoring the sky
               and the sides where the road edges appear.
    2. HSV   – converts the ROI to HSV. V (Value/brightness) ≤ 50 selects very
               dark/black pixels. Hue and Saturation are unrestricted so the
               detector doesn't care whether the obstacle is a dull black box or
               a glossy dark surface.
    3. Contours – finds solid blobs of dark pixels above OBSTACLE_MIN_AREA.
    4. Bottom-reach test – a contour only fires if its bounding-box bottom edge
               reaches the lower OBSTACLE_ROI_FRAC of the ROI, meaning the
               object is *close* (low in frame) not just far ahead.

    Tune these constants in the CONFIG section:
        OBSTACLE_MIN_AREA   smaller → more sensitive (more false positives)
        OBSTACLE_HSV_HI[2]  raise V threshold to catch dark-grey as well
        OBSTACLE_ROI_Y      raise the top of the ROI to look further ahead
        OBSTACLE_ROI_FRAC   lower to trigger on objects higher in the ROI
    """
    h, w = frame.shape[:2]

    # -- crop ROI ------------------------------------------------
    x1 = int(w * OBSTACLE_ROI_X[0]);  x2 = int(w * OBSTACLE_ROI_X[1])
    y1 = int(h * OBSTACLE_ROI_Y[0]);  y2 = h
    roi = frame[y1:y2, x1:x2]

    # -- HSV mask for very dark pixels ---------------------------
    hsv  = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)
    mask = cv2.inRange(hsv, OBSTACLE_HSV_LO, OBSTACLE_HSV_HI)
    ...
    for c in contours:
        if cv2.contourArea(c) < OBSTACLE_MIN_AREA:
            continue
        _x, _y, _w2, _h2 = cv2.boundingRect(c)
        if (_y + _h2) >= int(roi_h * OBSTACLE_ROI_FRAC):
            return True
    return False


def draw_obstacle_overlay(display, detected):
    """Draw the obstacle ROI rectangle and status badge on the display."""
    h, w = display.shape[:2]
    rx1 = int(w * OBSTACLE_ROI_X[0]);  rx2 = int(w * OBSTACLE_ROI_X[1])
    ry1 = int(h * OBSTACLE_ROI_Y[0])

    # ROI box
    colour = (0, 0, 255) if detected else (0, 255, 120)
    cv2.rectangle(display, (rx1, ry1), (rx2, h - 1), colour, 2)

    # Status badge
    if detected:
        cv2.rectangle(display, (rx1, ry1 - 34), (rx1 + 340, ry1 - 2), (0, 0, 200), -1)
        cv2.putText(display, "!!! BLACK OBSTACLE – THROTTLE STOP !!!",
                    (rx1 + 6, ry1 - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.60, (255, 255, 255), 2)
    else:
        cv2.rectangle(display, (rx1, ry1 - 26), (rx1 + 130, ry1 - 2), (0, 100, 0), -1)
        cv2.putText(display, "OBS: CLEAR",
                    (rx1 + 6, ry1 - 8),
...
while camera_running:
    ...
    # -- black obstacle check (runs on every frame, ~0.5 ms) ------
    obstacle_detected = black_obstacle_detected(frame)

    link.update(_s_deg, _thr, frame_ok=_fresh,
                frame_time=last_frame_time, obstacle=obstacle_detected)
    ...
    draw_obstacle_overlay(display, obstacle_detected)
