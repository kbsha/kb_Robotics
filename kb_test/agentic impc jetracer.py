...
def local_planner(state: VehicleState, prev_offset: float,
    ...
    for offset in CANDIDATE_OFFSETS:
        ...
        path = _generate_candidate_path(state.x, state.y, state.yaw, offset)
    ...
    if not feasible_found or best is None:
        ...
        fallback_path = _generate_candidate_path(state.x, state.y, state.yaw, prev_offset)
...
def render_dashboard(logs: List[StepLog], gif_path: str, png_path: str):
    ...
    def frame_update(i):
        ...
        vehicle_patch.set_xy((cx - VEH.width / 2 * math.sin(yaw),
                               cy + VEH.width / 2 * math.cos(yaw) - VEH.width))
